...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(UserProfile.getName(LoginActivity.this)!="")
        {
            showProgress(true);
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
           ...
           /* try {
            ...
            //UserProfile.setName(mEmail,LoginActivity.this);
            ArrayList<PostParam> PostParams = new ArrayList<PostParam>();
            PostParam post = new PostParam("check", "check");
            PostParams.add(post);
            if (!Connectivity.isNetworkAvailable(LoginActivity.this)) {
                return false;
            }
            JSONObject ResponseJSON = PostRequest.execute("http://onlinepetition.comli.com/onlinepetition/getpet.php", PostParams, null);
            ...
            try {
                    ...
                    if (ResponseJSON.getInt("status") == 200){
                        UserProfile.setName(mEmail,LoginActivity.this);
                        JSONArray jPets = ResponseJSON.getJSONObject("data").getJSONArray("response");
                        for (int i=0; i<jPets.length(); i++){
                            JSONObject jpet= jPets.getJSONObject(i);
                            String name = jpet.getString("name");
                            String head = jpet.getString("head");
                            String content = jpet.getString("content");
                            /*int up = jpet.getInt("upvote");
                            int down = jpet.getInt("downvote");*/
                            int up=0;
                            int down =0;
                            //Pet newpet = new Pet(name,head,content,up,down);
        ...
        protected void onPostExecute(final Boolean success) {
            ...
            if (success) {
                ...
                CharSequence text = "Network not available or Server Error";
